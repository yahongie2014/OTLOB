...
<div class="wrapper-page">
    ...
    <div class="m-t-40 card-box">
        <div class="text-center">
            <h4 class="text-uppercase font-bold m-b-0">إعادة تفعيل حسابك</h4>
            @if (session('status'))
            <div class="alert alert-success">
                {{ session('status') }}
            </div>
            @endif
            <p class="text-muted m-b-0 font-13 m-t-20" dir="rtl">ادخل بريدك الإلكتروني و سوف تصلك رساله التفعيل.  </p>
        </div>
        <div class="panel-body">
            <form class="form-horizontal" role="form" method="POST" action="{{ url('/verify/resend') }}">
                {{ csrf_field() }}
